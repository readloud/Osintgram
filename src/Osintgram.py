...
class Osintgram:
    ...
    def get_addrs(self):
        ...
        sort_addresses = sorted(list(address.items()), key=lambda p: p[1], reverse=True)
    ...
    def get_hashtags(self):
        ...
        if len(hashtags) > 0:
            ...
            ssort = sorted(list(hashtag_counter.items()), key=lambda value: value[1], reverse=True)
    ...
    def check_private_profile(self):
        if self.is_private and not self.following:
            ...
            send = eval(input("Do you want send a follow request? [Y/N]: "))
            if send.lower() == "y":
